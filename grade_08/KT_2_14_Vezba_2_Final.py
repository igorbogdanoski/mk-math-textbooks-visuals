...
class KT_2_14_Vezba_2_Final(TextbookScene):
    
    # --- 1. ФУНКЦИЈА ЗА МРЕЖА (GRID) ---
    def get_digit_at(self, char, row, col, center_point, color=MK_BLACK, size=70):
        dx = 0.8  # Растојание меѓу цифрите
        dy = 1.2  # Растојание меѓу редовите
    ...
    # --- 3. ФУНКЦИЈА ЗА ПОЗАЈМУВАЊЕ (BORROW) ---
    ...
    def construct(self):
        ...
        # ОПЕРАТОР +
        op_plus = MathTex("+", font_size=60).next_to(n2_int, LEFT, buff=0.6)
        ...
        # 5+7=12 (АНИМАЦИЈА НА ПРЕФРЛАЊЕ)
        r1_d2 = self.get_digit_at("2", 0, 2, c1).set_y(res1_y)
        self.play(Write(r1_d2))
        
        # Единицата „лета“ горе во следната колона
        carry_one = self.animate_carry(target_mob=n1_d1, val="1")
        
        # 2+2+1=5
        r1_d1 = self.get_digit_at("5", 0, 1, c1).set_y(res1_y)
        # Кратко означување што се собира
        indic = SurroundingRectangle(VGroup(carry_one, n1_d1, n2_d1), color=MK_GREEN, buff=0.05)
        self.play(Create(indic), run_time=0.5)
        self.play(Write(r1_d1), FadeOut(indic))
        ...
        # ---------------------------------------------------------
        # ИНФОРМАЦИЈА ЗА ВТОРИОТ ЧЕКОР
        # ---------------------------------------------------------
        info_text_1 = self.get_text("Втората заграда е: (2,34 - 9,145)", size=32, color=MK_BLACK)
        info_text_2 = self.get_text("Бидејќи 9,145 > 2,34 резултатот е НЕГАТИВЕН!", size=32, color=MK_RED)
        info_text_3 = self.get_math("-(9{,}145 - 2{,}34)", size=48, color=MK_RED)
        
        info_group = VGroup(info_text_1, info_text_2, info_text_3).arrange(DOWN, buff=0.3).move_to(RIGHT * 3)
        
        self.play(Write(info_text_1))
        self.wait(0.5)
        self.play(Write(info_text_2))
        self.play(Write(info_text_3))
        self.wait(2)
        self.play(FadeOut(info_group))

        # ---------------------------------------------------------
        # ЧЕКОР 2: ОДЗЕМАЊЕ (9,145 - 2,34)
        # ---------------------------------------------------------
        step2_title = self.get_text("Чекор 2", size=30, color=MK_BLUE, is_bold=True)
        c2 = RIGHT * 2 + DOWN * 0.5 
        
        # Поставување 9,145 горе
        # Ред 1
        ...
        # ОПЕРАТОР -
        op_minus = MathTex("-", font_size=60).next_to(m2_int, LEFT, buff=0.6)
        ...
        # 1-3 (АНИМАЦИЈА НА ПОЗАЈМУВАЊЕ)
        # Покажуваме дека не може 1-3
        indic_cant = SurroundingRectangle(VGroup(m1_d1, m2_d1), color=MK_ORANGE, buff=0.05)
        self.play(Create(indic_cant), run_time=0.5)
        self.play(FadeOut(indic_cant))
        
        # Позајмуваме од 9
        borrow_vis = self.animate_borrow(
            lender=m1_int, 
            borrower=m1_d1, 
            new_lender_val="8", 
            new_borrower_val="11"
        ...
        # 8-2=6 (Користиме 8 од borrow_vis, не 9)
        ...
        # ДОДАВАЊЕ НА ЗНАК МИНУС ВО РЕЗУЛТАТОТ
