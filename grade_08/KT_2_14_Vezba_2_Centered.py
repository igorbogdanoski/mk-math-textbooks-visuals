...
class KT_2_14_Vezba_2_Centered(TextbookScene):
    ...
    def construct(self):
        ...
        # --- СМАЛУВАЊЕ И ПОЗИЦИОНИРАЊЕ НА ЧЕКОР 1 (КОРЕГИРАНО) ---
        # Поместуваме помалку лево (LEFT * 2.5 наместо 4)
        block1 = VGroup(group1, phantom_zero, r1_d3, r1_d2, carry_one, r1_d1, r1_com, r1_int)
        self.play(block1.animate.scale(0.7).to_edge(DOWN, buff=0.5).shift(LEFT * 2.5))
        ...
        # --- СМАЛУВАЊЕ И ПОЗИЦИОНИРАЊЕ НА ЧЕКОР 2 (КОРЕГИРАНО) ---
        # Поместуваме помалку десно (RIGHT * 2.8 наместо >3.5)
        block2 = VGroup(group2, m2_zero, r2_d3, r2_d2, borrow_vis, r2_d1, r2_int, r2_com, res_neg_sign)
        self.play(block2.animate.scale(0.7).to_edge(DOWN, buff=0.5).shift(RIGHT * 2.8))
