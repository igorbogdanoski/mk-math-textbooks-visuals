...
class KT_2_14_Vezba_2_Perfect(TextbookScene):
    ...
    def construct(self):
        ...
        # ---------------------------------------------------------
        # ИНФОРМАЦИЈА ЗА ВТОРИОТ ЧЕКОР (КОРЕГИРАНО)
        # ---------------------------------------------------------
        # Намален фонт (size=26) и поместена група (scale=0.9)
        info_text_1 = self.get_text("Втората заграда е: (2,34 - 9,145)", size=26, color=MK_BLACK)
        info_text_2 = self.get_text("Бидејќи 9,145 > 2,34 резултатот е НЕГАТИВЕН!", size=26, color=MK_RED)
        info_text_3 = self.get_math("-(9{,}145 - 2{,}34)", size=38, color=MK_RED)
        
        info_group = VGroup(info_text_1, info_text_2, info_text_3).arrange(DOWN, buff=0.3)
        # Ја ставаме десно од Блок 1, но малку повнатре за да не се сече
        info_group.next_to(block1, RIGHT, buff=1.5).shift(UP * 2)
        ...
        # КОРЕКЦИЈА: .move_to(UP * 2.0) за да биде повисоко
        final_group = VGroup(calc_text, final_res).arrange(RIGHT, buff=0.2).move_to(UP * 2.0)
