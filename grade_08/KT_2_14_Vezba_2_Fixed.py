...
class KT_2_14_Vezba_2_Fixed(TextbookScene):
    ...
    def construct(self):
        ...
        # Смалување и поместување во аголот
        ...
        # ---------------------------------------------------------
        # ИНФОРМАЦИЈА ЗА ВТОРИОТ ЧЕКОР (КОРЕКЦИЈА НА ПОЗИЦИЈА)
        # ---------------------------------------------------------
        info_text_1 = self.get_text("Втората заграда: (2,34 - 9,145)", size=30, color=MK_BLACK)
        info_text_2 = self.get_text("Бидејќи 9,145 > 2,34 резултатот е НЕГАТИВЕН!", size=28, color=MK_RED)
        info_text_3 = self.get_math("-(9{,}145 - 2{,}34)", size=42, color=MK_RED)
        
        # Групираме и позиционираме централно-десно, без да бега од екран
        info_group = VGroup(info_text_1, info_text_2, info_text_3).arrange(DOWN, buff=0.3)
        info_group.move_to(RIGHT * 2 + UP * 1) # Безбедна позиција
        ...
        # ФИНАЛЕ
        # ---------------------------------------------------------
        # КЛУЧЕН МОМЕНТ: Тргање на старата задача за да нема преклопување
        self.play(FadeOut(problem_eq))
        
        calc_text = self.get_math("5{,}525 - (-6{,}805) = 5{,}525 + 6{,}805", size=45)
        final_res = self.get_math("= 12{,}33", size=45, color=MK_BLUE)
        
        # Позиционирање високо горе, каде што беше задачата
        final_group = VGroup(calc_text, final_res).arrange(RIGHT, buff=0.2).move_to(UP * 2.2)
